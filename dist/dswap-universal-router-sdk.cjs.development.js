...
var dswapV2Sdk = require('@miljan9602/dswap-v2-sdk');
var dswapV3Sdk = require('@miljan9602/dswap-v3-sdk');
var dswapRouterSdk = require('@miljan9602/dswap-router-sdk');
var dswapSdkCore = require('@miljan9602/dswap-sdk-core');
require('jsbi');
...
(function (RouterTradeType) {
  ...
  RouterTradeType["UnwrapWETH"] = "UnwrapWSEI";
...
}, _CHAIN_CONFIGS[dswapSdkCore.ChainId.SEI_MAINNET] = {
  router: '0x643770E279d5D0733F21d6DC03A8efbABf3255B4',
...
function encodeFeeBips(fee) {
  return dswapV3Sdk.toHex(fee.multiply(10000).quotient);
}

var REFUND_ETH_PRICE_IMPACT_THRESHOLD = /*#__PURE__*/new dswapSdkCore.Percent(50, 100);
...
var UniswapTrade = /*#__PURE__*/function () {
  ...
  _proto.encode = function encode(planner, _config) {
    ...
    var performAggregatedSlippageCheck = this.trade.tradeType === dswapSdkCore.TradeType.EXACT_INPUT && this.trade.routes.length > 2;
    ...
    if (routerMustCustody) {
      ...
      if (!!this.options.fee) {
        ...
        if (this.trade.tradeType === dswapSdkCore.TradeType.EXACT_OUTPUT) {
    ...
    if (this.inputRequiresWrap && (this.trade.tradeType === dswapSdkCore.TradeType.EXACT_OUTPUT || riskOfPartialFill(this.trade))) {
...
function addV2Swap(planner, _ref, tradeType, options, payerIsUser, routerMustCustody) {
  ...
  var trade = new dswapV2Sdk.Trade(route, tradeType == dswapSdkCore.TradeType.EXACT_INPUT ? inputAmount : outputAmount, tradeType);
  if (tradeType == dswapSdkCore.TradeType.EXACT_INPUT) {
  ...
  } else if (tradeType == dswapSdkCore.TradeType.EXACT_OUTPUT) {
...
function addV3Swap(planner, _ref2, tradeType, options, payerIsUser, routerMustCustody) {
  ...
  var trade = dswapV3Sdk.Trade.createUncheckedTrade({
    route: route,
    inputAmount: inputAmount,
    outputAmount: outputAmount,
    tradeType: tradeType
  });
  var path = dswapV3Sdk.encodeRouteToPath(route, trade.tradeType === dswapSdkCore.TradeType.EXACT_OUTPUT);
  if (tradeType == dswapSdkCore.TradeType.EXACT_INPUT) {
...
function addMixedSwap(planner, swap, tradeType, options, payerIsUser, routerMustCustody) {
  ...
  if (route.pools.length === 1) {
    if (route.pools[0] instanceof dswapV3Sdk.Pool) {
      return addV3Swap(planner, swap, tradeType, options, payerIsUser, routerMustCustody);
    } else if (route.pools[0] instanceof dswapV2Sdk.Pair) {
      return addV2Swap(planner, swap, tradeType, options, payerIsUser, routerMustCustody);
    } else {
      throw new Error('Invalid route type');
    }
  }
  var trade = dswapRouterSdk.MixedRouteTrade.createUncheckedTrade({
    route: route,
    inputAmount: inputAmount,
    outputAmount: outputAmount,
    tradeType: tradeType
  ...
  var sections = dswapRouterSdk.partitionMixedRouteByProtocol(route);
  ...
  for (var i = 0; i < sections.length; i++) {
    ...
    outputToken = dswapRouterSdk.getOutputOfPools(section, inputToken);
    var newRouteOriginal = new dswapRouterSdk.MixedRouteSDK([].concat(section), section[0].token0.equals(inputToken) ? section[0].token0 : section[0].token1, outputToken);
    var newRoute = new dswapRouterSdk.MixedRoute(newRouteOriginal);
    /// Previous output is now input
    inputToken = outputToken;
    var mixedRouteIsAllV3 = function mixedRouteIsAllV3(route) {
      return route.pools.every(function (pool) {
        return pool instanceof dswapV3Sdk.Pool;
      });
    };
    if (mixedRouteIsAllV3(newRoute)) {
      var path = dswapRouterSdk.encodeMixedRouteToPath(newRoute);
...
var RouterTradeAdapter = /*#__PURE__*/function () {
  ...
  RouterTradeAdapter.fromClassicQuote = function fromClassicQuote(quote) {
    ...
    var typedRoutes = route.map(function (subRoute) {
      ...
      var inputAmount = dswapSdkCore.CurrencyAmount.fromRawAmount(parsedCurrencyIn, rawAmountIn);
      var outputAmount = dswapSdkCore.CurrencyAmount.fromRawAmount(parsedCurrencyOut, rawAmountOut);
      ...
      return {
        routev3: isOnlyV3 ? new dswapV3Sdk.Route(subRoute.map(RouterTradeAdapter.toPool), parsedCurrencyIn, parsedCurrencyOut) : null,
        routev2: isOnlyV2 ? new dswapV2Sdk.Route(subRoute.map(RouterTradeAdapter.toPair), parsedCurrencyIn, parsedCurrencyOut) : null,
        mixedRoute: !isOnlyV3 && !isOnlyV2 ? new dswapRouterSdk.MixedRouteSDK(subRoute.map(RouterTradeAdapter.toPoolOrPair), parsedCurrencyIn, parsedCurrencyOut) : null,
        inputAmount: inputAmount,
        outputAmount: outputAmount
      };
    });
    return new dswapRouterSdk.Trade({
  ...
  RouterTradeAdapter.toCurrency = function toCurrency(isNative, token) {
    if (isNative) {
      return dswapSdkCore.Ether.onChain(token.chainId);
    }
  ...
  RouterTradeAdapter.toToken = function toToken(token) {
    ...
    return new dswapSdkCore.Token(chainId, address, parseInt(decimals.toString()), symbol, /* name */undefined, false, buyFeeBps ? ethers.BigNumber.from(buyFeeBps) : undefined, sellFeeBps ? ethers.BigNumber.from(sellFeeBps) : undefined);
  };
  RouterTradeAdapter.toPool = function toPool(_ref) {
    ...
    return new dswapV3Sdk.Pool(RouterTradeAdapter.toToken(tokenIn), RouterTradeAdapter.toToken(tokenOut), parseInt(fee), sqrtRatioX96, liquidity, parseInt(tickCurrent));
  };
...
RouterTradeAdapter.toPair = function (_ref2) {
  ...
  return new dswapV2Sdk.Pair(dswapSdkCore.CurrencyAmount.fromRawAmount(RouterTradeAdapter.toToken(reserve0.token), reserve0.quotient), dswapSdkCore.CurrencyAmount.fromRawAmount(RouterTradeAdapter.toToken(reserve1.token), reserve1.quotient));
};
...
//# sourceMappingURL=dswap-universal-router-sdk.cjs.development.js.map
